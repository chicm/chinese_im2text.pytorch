...
def build_vocab_old(imgs, params):
    count_thr = params['word_count_threshold']
...
def encode_captions(imgs, params, wtoi):
    ...
    for i, img in enumerate(imgs):
        ...
        for j, sentence in enumerate(img['caption']):
            #for sentence in s:
            #print('sentence:'+sentence)
            if len(sentence) < 1:
                print('empty caption:' + str(i))
                sentence = img['caption'][j-1]
...
def main(params):
    ...
    itow = {i + 1: w[0] for i, w in enumerate(vocab)}  # a 1-indexed vocab translation table
    wtoi = {w[0]: i + 1 for i, w in enumerate(vocab)}  # inverse table
    with open('data/word_counts.txt', 'w') as f:
        for w in vocab:
            f.write('{}, {}\n'.format(w[0], w[1]))
    #print('itow: {}'.format(len(itow)))
    #print('wtoi: {}'.format(len(wtoi)))
    #test = [(k[0],i) for i,k in enumerate(vocab)]
    #print(test[:10])
    #try:
    #    print(wtoi['滕'])
    #except KeyError:
    ...
    json.dump(out, open(params['output_json'], 'w'), ensure_ascii=False, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_h5', default='data/ai_challenger', help='output h5 file')
