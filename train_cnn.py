...
def train(opt):
    ...
    loader = get_img_loader('/home/chicm/ml/kgdata/ai/caption/train/caption_train_annotations_20170902.json', '/home/chicm/ml/kgdata/ai/caption/train/480', batch_size=4)
    ...
    if opt.finetune_cnn_after != -1 and epoch >= opt.finetune_cnn_after:
        ...
        cnn_optimizer = optim.Adam(cnn_model.parameters(), lr=opt.learning_rate)
        #print(cnn_model._modules)
    ...
    for data in loader:
        ...
        images, labels, masks = Variable(images.cuda()), Variable(labels.cuda()), Variable(masks.cuda())
        
        if opt.finetune_cnn_after != -1 and epoch >= opt.finetune_cnn_after:
            cnn_optimizer.zero_grad()
        cnn_out = cnn_model(images)
        #att_feats = cnn_out.permute(0, 2, 3, 1)
        att_feats = F.adaptive_avg_pool2d(cnn_out,[14,14]).squeeze().permute(0,2,3,1)
        fc_feats = cnn_out.mean(3).mean(2)
        #print(att_feats.size())
        #print('opt.use_att:', opt.use_att)
        ...
        fc_feats, att_feats = fc_feats.contiguous(), att_feats.contiguous()
        print(att_feats.size())
        print(fc_feats.size())
